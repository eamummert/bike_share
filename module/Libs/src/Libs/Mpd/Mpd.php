...
class Mpd 
{
	...
	const MPD_CMD_PLSWAPTRACK = "swap";
	const MPD_CMD_PLMOVETRACK = "move";
	...
	public $state;
	public $current_track_position;
	public $current_track_length;
	...
	public $uptime;
	public $playtime;
	public $db_last_refreshed;
	public $num_songs_played;
	public $playlist_count;
	...
	// Misc Other Vars	
	public $mpd_class_version = "1.2";

	public $debugging   = FALSE;    // Set to TRUE to turn extended debugging on.
	public $errStr      = "";       // Used for maintaining information about the last error message
	...
	function SendCommand($cmdStr, $arg1 = "", $arg2 = "") 
	{
		...
		else 
		{
			...
			while (!feof($this->mpd_sock)) 
			{
				...
				if (strncmp(self::MPD_RESPONSE_ERR, $response, strlen(self::MPD_RESPONSE_ERR)) == 0) 
				{
					list($junk, $errTmp) = split(self::MPD_RESPONSE_ERR . " ", $response);
	...
	function SetVolume($newVol) 
	{
		...
		{
            if (!is_null($ret = $this->SendCommand(self::MPD_CMD_SETVOL,$newVol))) $this->volume = $newVol;
        } 
		else 
		{
			...
			else 
			{
                ...
                if (!is_null($ret = $this->SendCommand(self::MPD_CMD_VOLUME,$modifier))) $this->volume = $newVol;
    		}
        }
	...
	function PLAdd($fileName) 
	{
		...
		if (!is_null($resp = $this->SendCommand(self::MPD_CMD_PLADD,$fileName))) $this->RefreshInfo();
	...
	function PLMoveTrack($origPos, $newPos) 
	{
		...
		if (!is_null($resp = $this->SendCommand(self::MPD_CMD_PLMOVETRACK,$origPos,$newPos))) $this->RefreshInfo();
	...
	function PLShuffle() 
	{
		...
		if (!is_null($resp = $this->SendCommand(self::MPD_CMD_PLSHUFFLE))) $this->RefreshInfo();
	...
	function PLLoad($file) 
	{
		...
		if (! is_null($resp = $this->SendCommand(self::MPD_CMD_PLLOAD,$file))) $this->RefreshInfo();
	...
	function PLClear() 
	{
		...
		if (!is_null($resp = $this->SendCommand(self::MPD_CMD_PLCLEAR))) $this->RefreshInfo();
	...
	function PLRemove($id) 
	{
		...
		if (!is_null($resp = $this->SendCommand(self::MPD_CMD_PLREMOVE,$id))) $this->RefreshInfo();
	...
	function DBRefresh() 
	{
		...
		$resp = $this->SendCommand(self::MPD_CMD_REFRESH);
	...
	function Play() 
	{
		...
		if (!is_null($rpt = $this->SendCommand(self::MPD_CMD_PLAY))) $this->RefreshInfo();
	...
	function Stop() 
	{
		...
		if (!is_null($rpt = $this->SendCommand(self::MPD_CMD_STOP))) $this->RefreshInfo();
	...
	function Pause() 
	{
		...
		if (!is_null($rpt = $this->SendCommand(self::MPD_CMD_PAUSE))) $this->RefreshInfo();
	...
	{ 
		...
		if (!is_null($rpt = $this->SendCommand(self::MPD_CMD_PLAY,$idx))) $this->RefreshInfo();
	...
	{ 
		...
		if (!is_null($rpt = $this->SendCommand(self::MPD_CMD_SEEK,$track,$pos))) $this->RefreshInfo();
	...
	function Next() 
	{
		...
		if (!is_null($rpt = $this->SendCommand(self::MPD_CMD_NEXT))) $this->RefreshInfo();
	...
	function Previous() 
	{
		...
		if (!is_null($rpt = $this->SendCommand(self::MPD_CMD_PREV))) $this->RefreshInfo();
	...
	{
		$types = [
			self::MPD_SEARCH_ARTIST,
			self::MPD_SEARCH_ALBUM,
			self::MPD_SEARCH_TITLE,
		];

		$results = [];
		foreach ($types as $type)
		{
			$results[$type] = $this->search($type, $string);
		}
	...
	*/
	...
	function RefreshInfo() 
	{
		...
		if (($this->state == self::MPD_STATE_PLAYING) || ($this->state == self::MPD_STATE_PAUSED)) 
		{
	...
	function GetStatistics() 
	{
		...
		$stats = $this->SendCommand(self::MPD_CMD_STATISTICS);
		if (!$stats) 
		{
			return NULL;
		} 
	...
	function GetStatus() 
	{
		...
		$status = $this->SendCommand(self::MPD_CMD_STATUS);
		if (!$status) 
		{
			return NULL;
		} 
	...
	function GetVolume() 
	{
		...
		$volLine = $this->SendCommand(self::MPD_CMD_STATUS);
		if (!$volLine) 
		{
			return NULL;
		} 
	...
	function GetPlaylist() 
	{
		...
		$resp = $this->SendCommand(self::MPD_CMD_PLLIST);
